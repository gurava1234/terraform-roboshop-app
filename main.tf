...
resource "null_resource" "component" {
    ...
    triggers = {
        instance_ids = module.component.id
    }

    # Bootstrap script can run on any instance of the cluster
    # so we just choose the first in thius case
    connection {
...
resource "null_resource" "component_delete" {
    #changes to any instance of the cluster requires re-provisioning
    triggers = {
        instance_ids = aws_ami_from_instance.component.id
    }

    provisioner "local_exec" {
    # Bootstrap script called with private_ip of each node in the cluster
      command = "aws ec2 terminate_instances --instance-ids ${module.component.id}"
    }
...
resource "aws_autoscaling_policy" "component" {
  ...
  target_tracking_configuration {
    predefined_metric_specification {
      predefined_metric_type = "ASSGAverageCPUUtilization"
    }
    target_value = 5.0
  }
}
